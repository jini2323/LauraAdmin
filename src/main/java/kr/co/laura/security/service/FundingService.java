...
public class FundingService {
	...
	*/
	public List<Long> top6dAchieveRates() {
	    // 1. 펀딩 모금액이 가장 높은 순 6개 가져오기
	    List<FundingDTO> top6FundingList = qFunRepositoryImpl.showTop6FundingsByFunMoney();

	    // 2. 그 6개의 펀딩의 달성률을 계산
	    List<Long> achievementRates = qFunRepositoryImpl.calFundingAchieveRates(top6FundingList);

	    // 3. 만약 achievementRates가 null이면 모든 요소가 0인 리스트를 생성
	    if (achievementRates == null) {
	        achievementRates = new ArrayList<>();
	        for (int i = 0; i < 6; i++) {
	            achievementRates.add(0L);
	        }
	    }
	    //더미 데이터 넣기 전에는 주석 
	   // System.out.println("펀딩 달성률 계산: " + achievementRates.get(0));
